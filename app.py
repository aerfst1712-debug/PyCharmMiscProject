...
ADMIN_PASSWORD = '1234'  # 🔑 คุณสามารถเปลี่ยนรหัสผ่านแอดมินตรงนี้ได้ตามใจชอบครับ
...
def update_db_structure():
    ...
    # 2. สร้างตารางบันทึกประวัติ Stock Logs (ถ้ายังไม่มี)
    conn.execute('''
...
@app.route('/')
def home():
    # ตรวจสอบสิทธิ์แอดมินผ่านรหัสผ่านที่ถูกต้องแทน ?admin=true แบบลอยๆ
    ...
    total_items = len(products) if not (search_query or category_filter) else \
    conn.execute('SELECT COUNT(*) FROM components').fetchone()[0]
    total_stock = conn.execute('SELECT SUM(stock) FROM components').fetchone()[0] or 0
    low_stock_list = [p['name'] for p in products if (p['stock'] or 0) < 5 and (p['stock'] or 0) > 0]
    out_of_stock_list = [p['name'] for p in products if (p['stock'] or 0) == 0]
...
def add_product():
    ...
    if request.method == 'POST':
        ...
        # บันทึกประวัติการเพิ่มชิ้นส่วนใหม่
        now_str = datetime.now().strftime('%d/%m/%b %H:%M:%S')
        conn.execute('INSERT INTO stock_logs (component_name, action_type, timestamp) VALUES (?, ?, ?)',
...
def reset_db():
    ...
    conn.close()
    update_db_structure()
    return "รีเซ็ตและเพิ่มแดชบอร์ดพร้อมตัวอย่างคลังข้อมูลสำเร็จแล้ว!"
