...
def update_db_structure():
    ...
    if cursor.fetchone()[0] == 0:
        default_components = [
            ('IC LM358', 'Integrated Circuit', '50 บาท', 'https://th.rs-online.com/images/F4852924-01.jpg', 'ไอซีขยายสัญญาณ Low Power Dual Operational Amplifier', 'ขาใช้งาน: 8 พิน, แรงดันไฟเลี้ยง: 3V-32V', 100, 'https://www.ti.com/lit/ds/symlink/lm358.pdf'),
            ('Arduino Uno R3', 'Microcontroller', '250 บาท', 'https://docs.arduino.cc/static/29849b29d499ec249f056bc293d07ec5/A000066_featured.jpg', 'บอร์ดไมโครคอนโทรลเลอร์ยอดนิยม', 'ชิปหลัก: ATmega328P, แรงดัน: 5V', 12, 'https://docs.arduino.cc/resources/datasheets/A000066-datasheet-pdf')
        ]
...
def home():
    ...
    conn.close()
    return render_template('index.html', products=products, is_admin=is_admin, total_items=total_items, total_stock=total_stock, low_stock_count=low_stock, out_of_stock_count=out_stock, chart_labels=chart_labels, chart_data=chart_data, logs=logs)
...
def checkout():
    ...
    for item in cart:
        ...
        conn.execute('INSERT INTO stock_logs (component_name, action_type, timestamp) VALUES (?, ?, ?)', (item['name'], f"เบิกจ่าย {item['qty']} ชิ้น (Order: {order_id})", datetime.now().strftime('%H:%M:%S')))
    conn.commit()
    conn.close()
